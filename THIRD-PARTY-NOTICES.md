# Third-Party Notices

This repository uses the following third-party packages and dependencies:

---

## Source Generator Project

### Microsoft.CodeAnalysis.Analyzers
- **Version:** 4.14.0
- **License:** MIT
- **Copyright:** .NET Foundation and Contributors
- **Repository:** https://github.com/dotnet/roslyn
- **Purpose:** Provides analyzers for developing Roslyn analyzers and source generators

### Microsoft.CodeAnalysis.CSharp
- **Version:** 5.0.0
- **License:** MIT
- **Copyright:** .NET Foundation and Contributors
- **Repository:** https://github.com/dotnet/roslyn
- **Purpose:** Roslyn C# compiler API for code analysis and generation

---

## Test Projects

### Microsoft.NET.Test.Sdk
- **Version:** 18.3.0
- **License:** MIT
- **Copyright:** Microsoft Corporation
- **Repository:** https://github.com/microsoft/vstest
- **Purpose:** Test platform for running unit tests

### xunit.v3
- **Version:** 3.2.2
- **License:** Apache-2.0
- **Copyright:** .NET Foundation and Contributors
- **Repository:** https://github.com/xunit/xunit
- **Purpose:** Unit testing framework for .NET

### xunit.runner.visualstudio
- **Version:** 3.1.5
- **License:** Apache-2.0
- **Copyright:** .NET Foundation and Contributors
- **Repository:** https://github.com/xunit/visualstudio.xunit
- **Purpose:** Visual Studio test runner for xUnit

### coverlet.collector
- **Version:** 8.0.0
- **License:** MIT
- **Copyright:** tonerdo and Contributors
- **Repository:** https://github.com/coverlet-coverage/coverlet
- **Purpose:** Code coverage collector for .NET

---

## Snapshot Test Project (Additional)

### Verify.XunitV3
- **Version:** 31.13.2
- **License:** MIT
- **Copyright:** Simon Cropp
- **Repository:** https://github.com/VerifyTests/Verify
- **Purpose:** Snapshot testing library for xUnit v3

### Verify.SourceGenerators
- **Version:** 2.5.0
- **License:** MIT
- **Copyright:** Simon Cropp
- **Repository:** https://github.com/VerifyTests/Verify.SourceGenerators
- **Purpose:** Source generator-specific extensions for Verify snapshot testing

---

## License Summary

All dependencies use permissive open-source licenses (MIT or Apache-2.0) that allow:
- Commercial use
- Modification
- Distribution
- Private use

For complete license texts, please refer to each package's repository listed above.

---

*This file was generated to comply with open-source attribution requirements and to acknowledge the excellent work of the .NET community.*
